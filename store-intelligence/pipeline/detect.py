...
class DetectionPipeline:
    ...
    def process_video(self, video_path: str, fps: int = 15) -> int:
        ...
        logger.info("Processing %s as camera %s", video_name, camera_id)
        ...
        logger.info(
            "Video %s complete: %s events (processed %s frames)",
            video_name,
            event_count,
            processed_frames,
        )
    ...
    def process_all_videos(self) -> int:
        ...
        for video_path in video_files:
            try:
                events = self.process_video(str(video_path))
                total_events += events
            except Exception as e:
        ...
        logger.info(
            "Pipeline complete: %s total events -> %s",
            total_events,
